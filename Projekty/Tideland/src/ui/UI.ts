...
const icon = (id: ItemId, cls = ''): string => `<img class="item-art ${cls}" src="/assets/icons/${id}.svg" alt="${esc(ITEMS[id].displayName)}" draggable="false">`;
  ...
  constructor(container: HTMLElement, actions: UIActions) {
    ...
    this.root.innerHTML = `
      <section class="screen menu-screen" data-view="menu" aria-label="Main menu">
        <header class="menu-masthead"><a class="brand-mark" href="#" aria-label="Tideland home">${mark}</a><span>AN OPEN WORLD<br>SURVIVAL EXPERIENCE</span><div class="edition"><i></i> EARLY ACCESS <b>01</b></div></header>
        ...
        <footer class="menu-footer"><span>INDEPENDENT SURVIVAL SANDBOX <i>/</i> BUILD 0.1.0</span><button data-action="help" class="text-button">CONTROLS <span>↗</span></button><span class="local-save-status"><i></i> LOCAL WORLD · SOLO</span></footer>
      </section>
      ...
      <section class="screen settings-screen" data-view="settings" aria-label="Settings"><header class="overlay-header"><div class="small-brand">${mark}<span>TIDELAND</span><i>/</i><span class="muted">SETTINGS</span></div><button class="close-button" data-action="settingsBack">BACK <span>×</span></button></header><div class="settings-content"><div class="settings-intro"><div class="eyebrow">MAKE YOURSELF AT HOME</div><h2>LIVE<br>PREVIEW<span>.</span></h2><p>Adjust the camera and watch the island.<br>Changes apply immediately and save automatically.</p></div><div class="settings-controls"><h3>CONTROLS & CAMERA</h3>${this.slider('sensitivity','Mouse sensitivity',0.2,3,0.1)}${this.slider('fov','Field of view <small>Horizontal at 16:9 · wider screens show more sides</small>',60,100,1)}<h3>AUDIO</h3>${this.slider('masterVolume','Master volume',0,1,0.01)}${this.slider('effectsVolume','Effects volume',0,1,0.01)}<h3>GRAPHICS</h3><div class="setting-row quality-row"><label>Graphics quality<small>Vegetation, shadows & rendering resolution</small></label><div class="quality-options"><button data-quality="low">LOW</button><button data-quality="medium">MEDIUM</button><button data-quality="high">HIGH</button></div></div><div class="save-reset-row"><span>LOCAL SAVE DATA<small>Remove your saved island and progress.</small></span><button class="danger-button" data-action="reset">RESET SAVE</button></div><div class="reset-confirm" hidden><span>This permanently removes the saved world.</span><button data-action="resetConfirm">DELETE SAVE</button><button data-action="resetCancel">CANCEL</button></div></div></div></section>
