...
export class GameApp {
  ...
  private updateBuild(){
    ...
    if(c.valid&&c.pieceType==='foundation'){
      for(const prop of this.environment.colliders){if(prop.nodeId&&this.simulation.state.nodeChanges[prop.nodeId]===0)continue;if(Math.abs(prop.position.x-c.position.x)<BUILD.SIZE/2+prop.halfExtents.x-.15&&Math.abs(prop.position.z-c.position.z)<BUILD.SIZE/2+prop.halfExtents.z-.15&&prop.position.y+prop.halfExtents.y>c.position.y&&prop.position.y-prop.halfExtents.y<c.position.y+BUILD.FOUNDATION_HEIGHT+1){c.valid=false;c.reason='Obstructed by a tree or rock';break;}}
    }
  ...
  private syncStations(){const stations=ensureProgression(this.simulation.state).stations;this.stationRenderer.sync(stations);for(const s of stations)if(!this.stationIds.has(s.id)){this.stationIds.add(s.id);this.physics.setStructure(s.id,this.stationRenderer.boxes(s));this.interactions.register({id:s.id,kind:'station',object:this.stationRenderer.objects.get(s.id)!,position:()=>s.position,enabled:()=>true,info:()=>({title:STATIONS[s.kind].name,action:'OPEN',key:'E'}),interact:()=>{this.openStation=s.id;this.setScreen('station');this.stationUI.open(s,this.simulation.state.inventory);}});}}
